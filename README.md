...
- [DDPGAgent](reinforce_transmon/src/rl/agent.py) **Class**: Includes methods to select actions, store transitions in a replay buffer, train actor and critic networks, update soft target networks, and save/load model weights.
  ...
  - [get_action()](reinforce_transmon/src/rl/agent.py#L198) **Method**: Get an action from the Policy network and add Gaussian noise for exploration.
...
```bash
conda env create -f cpu_environment.yml && conda activate rl-ddpg
```

## GPU Environment

```bash
conda env create -f gpu_environment.yml && conda activate rl-ddpg
```
